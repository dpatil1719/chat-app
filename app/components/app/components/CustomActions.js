 ...
 * - onSend: function(payload) that writes message to Firestore (already wired in your app)
...
const CustomActions = ({ storage, uid, onSend, wrapperStyle, iconTextStyle }) => {
  ...
  const uploadAndSendImage = async (imageURI) => {
    try {
      // Convert to Blob via fetch (works in Expo + RN)
      ...
      // Send an image message (Chat.js onSend writes this to Firestore)
      onSend({ image: url });
    } catch (e) {
  ...
  const pickImage = async () => {
    try {
      ...
      // Avoid specifying mediaTypes to maximize compatibility across ImagePicker versions
